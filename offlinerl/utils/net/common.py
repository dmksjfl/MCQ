...
from offlinerl.utils.net.spectral_norm import SpectralNorm as SN
...
def miniblock(
    ...
    if spectral_normalization:
        ret: List[nn.modules.Module] = [SN(nn.Linear(inp, oup))]
    else:
    ...
    # dropout
    # ret += [nn.modules.dropout.Dropout(0.01)]
    return ret
...
class Net(nn.Module):
    ...
    def __init__(
        ...
        model = miniblock(input_size, hidden_layer_size, norm_layer, spectral_normalization)

        for i in range(layer_num):
            model += miniblock(
                hidden_layer_size, hidden_layer_size, norm_layer, spectral_normalization)
        ...
        if self.output_shape:
            if spectral_normalization:
                model +=  [SN(nn.Linear(hidden_layer_size, output_shape))]
            else:
